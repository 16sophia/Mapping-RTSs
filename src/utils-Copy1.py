...
def analyze_preds(preds, tile_id, threshold_mask = 0.5):
    ...
    for i in range(len(preds[tile_id]["labels"])): # Iterate through RTS/ tile
        ...
        mean_intensity = np.mean(RTS_mask[0][mask_boolean >=threshold_mask ])
...
def similarity_mask(results, pred_mask, targ_mask, mask_thr):
    ...
    for pred_id, targ_id in enumerate(matched):
        if targ_id>-1: # Match 
            ...
            accuracy, precision, recall, f1 = performance_metric(labelled.flatten(), predicted.flatten())
    ...
    return acc_pixel_, precision_pixel_, recall_pixel_, f1_pixel_ 
...
'''
        ...
        #### similarity_mask(matched_elements)----------------------------------------------------------------------------------------------------------------------------------
        acc_pixel_, precision_pixel_, recall_pixel_, f1_pixel_ = similarity_mask(results, pred_mask, targ_mask, mask_thr)
        ...
        if get_TP_ind:
            return acc_pixel_, precision_pixel_, recall_pixel_, f1_pixel_, acc, precision, recall, F1, RTS_TP_ind
        else:
            return acc_pixel_, precision_pixel_, recall_pixel_, f1_pixel_, acc, precision, recall, F1
...
def similarity_RTS(pred_mask, targ_mask, src_boxes, pred_boxes, iou_thresholds = [0.5], get_TP_ind = False, mask_thr=0.5):
    ...
    if total_gt > 0 and total_pred > 0:
        ...
        for t in iou_thresholds:
            if get_TP_ind:
                acc_pixel_, precision_pixel_, recall_pixel_, f1_pixel_, acc, precision, recall, F1, RTS_TP = iou_acc(total_gt , pred_mask, targ_mask, mask_thr, src_boxes, pred_boxes, t, get_TP_ind)
                RTS_TP_t = RTS_TP_t+RTS_TP     
                
            else:
                acc_pixel_, precision_pixel_, recall_pixel_, f1_pixel_, acc, precision, recall, F1 = iou_acc(total_gt , pred_mask, targ_mask, mask_thr, src_boxes, pred_boxes, t)
            # Metrics / RTS on image level
            acc_t.append(acc)
        ...
        if get_TP_ind:
            return acc_pixel_, precision_pixel, recall_pixel, f1_pixel, acc_avg, precision_avg, recall_avg, F1_avg, RTS_TP_t
        else:
            return acc_pixel_, precision_pixel, recall_pixel, f1_pixel, acc_avg, precision_avg, recall_avg, F1_avg

    # There are no labelled RTS 
    elif total_gt == 0: # TP= 0
        if total_pred > 0:
            if get_TP_ind: # all TP indices of predicted RTS are set to -1
                return np.nan, np.nan,np.nan,np.nan,torch.tensor(0.),torch.tensor(0.),torch.tensor(0.),torch.tensor(0.), [-1]* len(total_pred)
            else:
                return np.nan, np.nan, np.nan, np.nan, torch.tensor(0.),torch.tensor(0.),torch.tensor(0.),torch.tensor(0.)
        else:
            if get_TP_ind: 
                return np.nan, np.nan, np.nan, np.nan, torch.tensor(0.),torch.tensor(0.),torch.tensor(0.),torch.tensor(0.), [] # no predicted RTS -> TP list is empty
