...
def get_transform(flip = False):
    '''
    Generates a data transformation pipeline:
    
    Mandatory transform:
        T.PILToTensor(): converts a PIL (Python Imaging Library) image to a PyTorch tensor
        
    Optional transforms:    
        T.ConvertImageDtype(torch.float): converts the tensor data type to torch.float.
        T.RandomHorizontalFlip(0.5): horizontally flips the image with a 50% probability. 
    Input: train= boolean value, training data (train=True), validation/testing data (train=False).
    Return: T.Compose(transforms)= Transformaiton function / composition of all the transformations in the transforms list
        Application: train_transforms = get_transform(train=True) # Get transformations function
                     transformed_train_data = train_transforms(image)
    '''
...
class ToTensor(object):
    '''
    Transforms image and target from np.array to tensor & transforms dimension from (h, w, c) to (c,h,w) with torchvision transform function
    '''
    def __call__(self, image, target):
