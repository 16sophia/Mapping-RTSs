...
class Model(pl.LightningModule):
    ...
    def __init__(self, num_classes, pretrained_network, config, change_min_box_axis = False):
        ...
        # modify model with monkey patching to fit mean, std, number of channels, image size
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Loss
        self.sum_loss = sum(loss_dict.values())
        ...
        self.log("sum_loss_training", self.sum_loss, batch_size=batch_size) # Sum of all losses
        self.log_dict(loss_dict, batch_size=batch_size) # dictionary containing all different losses
        
        wandb.log({"sum_loss_training": self.sum_loss})
        ...
        for labels, output in zip(targets,preds):
            acc_pixel, precision_pixel, recall_pixel, f1_pixel, accuracy_image, precision_image, recall_img, F1_img = torch.Tensor(utils.similarity_RTS(output["masks"], labels["masks"], labels["boxes"],output["boxes"])).to(self.device_)
            # Performance metrics on image level
            accuracy_.append(accuracy_image)
            precision_.append(precision_image)
            recall_.append(recall_img)
            F1_.append(F1_img)
            # Performance metrics on RTS level
        ...
        # Performance metrics on image level
        self.train_accuracy = torch.mean(torch.stack(accuracy_)) 
        self.train_precision = torch.mean(torch.stack(precision_))
        self.train_recall = torch.mean(torch.stack(recall_)) 
        self.train_F1 = torch.mean(torch.stack(F1_))
        self.log("Accuray_train", self.train_accuracy, batch_size=batch_size)
        self.log("Precision_train", self.train_precision, batch_size = batch_size)
        self.log("Recall_train", self.train_recall, batch_size=batch_size)
        self.log("F1_train", self.train_F1, batch_size = batch_size)
        wandb.log({"Accuray_train": self.train_accuracy})
        wandb.log({"Precision_train": self.train_precision})
        wandb.log({"Recall_train": self.train_recall})
        wandb.log({"F1_train": self.train_F1})        
        ...
        self.log("Mask_accuracy_train", accuracy_pixel, batch_size=batch_size)
        self.log("Mask_precision_train", precision_pixel, batch_size=batch_size)
        self.log("Mask_recall_train", recall_pixel, batch_size=batch_size)
        self.log("Mask_F1_train", F1_pixel, batch_size=batch_size)
        wandb.log({"Mask_accuray_train": accuracy_pixel})
        wandb.log({"Mask_precision_train": precision_pixel})
        wandb.log({"Mask_recall_train": recall_pixel})
        wandb.log({"Mask_F1_train": F1_pixel})
        

        return {"loss": self.sum_loss, "log": loss_dict}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Get loss dictionary
        self.detector.train()
        loss_dict = self.detector(img, targets)
        #print(loss_dict)
        for key, value in loss_dict.items():
            wandb.log({f'validation_{key}': value})
        ...
        for labels, prediction in zip(targets,preds):
            acc_pixel, precision_pixel, recall_pixel, f1_pixel, accuracy_image, precision_image, recall_img, F1_img = torch.Tensor(utils.similarity_RTS(prediction["masks"], labels["masks"], labels["boxes"],prediction["boxes"])).to(self.device_)
            # Performance metrics on image level
            accuracy_.append(accuracy_image)
            precision_.append(precision_image)
            recall_.append(recall_img)
            ...
            # RTS have been predicted and RTS have been labelled 
                ...
                # Calculate loss to match with torchvision: https://github.com/pytorch/vision/blob/e12d200c97d7aab668b976e92b46513c9ca7a0d8/torchvision/models/detection/roi_heads.py#L123
        ...
        wandb.log({'validation_classification_loss': classification_loss})
        wandb.log({'validation_box_regression_loss': box_regression_loss})
        wandb.log({'validation_mask_loss': mask_loss})
        wandb.log({'validation_iou_RTS': iou_RTS})
        
        
        # Performance metrics on image level
        self.validation_accuracy = torch.mean(torch.stack(accuracy_)) 
        self.validation_precision = torch.mean(torch.stack(precision_))
        self.validation_recall = torch.mean(torch.stack(recall_)) 
        self.validation_F1 = torch.mean(torch.stack(F1_))
        self.log("validation_accuracy", self.validation_accuracy, batch_size=batch_size)
        self.log("validation_precision", self.validation_precision, batch_size = batch_size)
        self.log("validation_recall", self.validation_recall, batch_size=batch_size)
        self.log("validation_F1", self.validation_F1, batch_size = batch_size)
        wandb.log({"validation_accuracy": self.validation_accuracy})
        wandb.log({"validation_precision": self.validation_precision})
        wandb.log({"validation_recall": self.validation_recall})
        wandb.log({"validation_F1": self.validation_F1})
        ...
        recall_pixel = torch.nanmean(torch.tensor(recall_pixel_, dtype=torch.float))
        F1_pixel = torch.nanmean(torch.tensor(F1_pixel_, dtype=torch.float))
        self.log("Mask_accuracy_validation", accuracy_pixel, batch_size=batch_size)
        self.log("Mask_precision_validation", precision_pixel, batch_size=batch_size)
        self.log("Mask_recall_validation", recall_pixel, batch_size=batch_size)
        self.log("Mask_F1_validation", F1_pixel, batch_size=batch_size)
        wandb.log({"Mask_accuray_validation": accuracy_pixel})
        wandb.log({"Mask_precision_validation": precision_pixel})
        wandb.log({"Mask_recall_validation": recall_pixel})
        wandb.log({"Mask_F1_validation": F1_pixel})
    ...
    def test_step(self, img_org, targ_org, preds, iou_thresholds= [.5], get_TP_ind = False):
        ...
        batch_size = len(img)
        preds = self.forward(img)
        get_TP_ind: We want to retrieve boolean index wheter predicted RTS is TP
        ...
        for labels, prediction in zip(targets,preds): # iterate trough tile / batch
            if get_TP_ind:
                acc_pixel, precision_pixel, recall_pixel, f1_pixel, accuracy_image, precision_image, recall_img, F1_img, TP_ind = utils.similarity_RTS(prediction["masks"], labels["masks"], labels["boxes"],prediction["boxes"], iou_thresholds, get_TP_ind)
                RTS_TP_ = RTS_TP_ + TP_ind
                
            else:
                acc_pixel, precision_pixel, recall_pixel, f1_pixel, accuracy_image, precision_image, recall_img, F1_img = torch.Tensor(utils.similarity_RTS(prediction["masks"], labels["masks"],labels["boxes"],prediction["boxes"], iou_thresholds)).to(self.device_)
            # Performance metrics on image level
            accuracy_.append(accuracy_image)
            precision_.append(precision_image)
            recall_.append(recall_img)
        ...
        # Performance metrics on image level
        test_accuracy = np.mean([value.cpu() for value in accuracy_])
        test_precision = np.mean([value.cpu() for value in precision_])
        test_recall = np.mean([value.cpu() for value in recall_])
        test_F1 = np.mean([value.cpu() for value in F1_])
        # Performance metrics on RTS level
